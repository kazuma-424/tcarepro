...
class Score:
    ...
    def graph_make(self, session_code):
        ...
        # SQL検索
        cur.execute(
            'SELECT * FROM autoform_shot WHERE session_code ="' + session_code + '"'
        )
        ...
        pyplot.title(
            self.time + "に" + str(self.count) + "回実行したグラフ", fontname="Hiragino sans"
        )
...
def sql_reservation():
    ...
    cur = conn.cursor()
    cur.execute(
        'SELECT contact_url,sender_id,scheduled_date,callback_url,worker_id FROM contact_trackings WHERE status = "自動送信予定"'
    )
        ...
        if (
            ...
            if (num - sabun) >= 4:
                ...
                if int(minute) > 59:
                    ...
                    print("new")
                    schedule.every().day.at(
                        str(int(hour) + 1) + ":" + str(int(newminute) * -1).zfill(2)
                    ).do(
                        boot,
                        trigger["url"],
                        trigger["sender_id"],
